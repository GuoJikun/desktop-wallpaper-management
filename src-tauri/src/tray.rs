...
pub fn create_tray(app: &mut App) -> tauri::Result<()> {
    ...
    let installed = MenuItem::with_id(app, "installed", "本地壁纸", true, None::<&str>)?;
    let store = MenuItem::with_id(app, "store", "壁纸中心", true, None::<&str>)?;
    let menu = MenuBuilder::new(app)
        .items(&[&installed, &store, &auto_start, &quit])
        .build()?;
    ...
    let _ = TrayIconBuilder::with_id("tray")
        ...
        .on_menu_event(move |app, event| match event.id.as_ref() {
            ...
            "installed" => {
                println!("壁纸中心");
                // 打开设置窗口
                if let Ok(webview_window) = helper::get_webview_window(app, "installed", "/installed")
                {
                    let _ = webview_window.set_title("本地壁纸");
            ...
            "store" => {
                println!("壁纸中心");
                // 打开设置窗口
                if let Ok(webview_window) = helper::get_webview_window(app, "store", "/store")
                {
                    let _ = webview_window.set_title("壁纸中心");
                    let _ = webview_window.show();
                }
            }
            "auto_start" => {
        ...
        .on_tray_icon_event(|tray, event| {
            ...
            } = event
            {
                ...
                if let Some(window) = app.get_webview_window("installed") {
                    let _ = window.show();
